...
class DQModel(keras.Model):
    def __init__(self, hidden_size: int, num_actions: int, dueling: bool):
        ...
        self.conv1 = keras.layers.Conv2D(16, (8, 8), (4, 4), activation=tf.compat.v1.keras.layers.PReLU)
        self.conv2 = keras.layers.Conv2D(32, (4, 4), (2, 2), activation=tf.compat.v1.keras.layers.PReLU)
        self.flatten = keras.layers.Flatten()
        self.adv_dense = keras.layers.Dense(hidden_size, activation=tf.compat.v1.keras.layers.PReLU,
                                         kernel_initializer=keras.initializers.he_normal())
        self.adv_out = keras.layers.Dense(num_actions,
                                          kernel_initializer=keras.initializers.he_normal())
        if dueling:
            self.v_dense = keras.layers.Dense(hidden_size, activation=tf.compat.v1.keras.layers.PReLU,
                                         kernel_initializer=keras.initializers.he_normal())
...
primary_network.compile(optimizer=keras.optimizers.Adam(), loss=tf.compat.v1.keras.losses.Huber())
