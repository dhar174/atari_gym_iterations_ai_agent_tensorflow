...
class DQModel(keras.Model):
    def __init__(self, hidden_size: int, num_actions: int, dueling: bool):
        ...
        self.pooling_layer1 = tf.keras.layers.MaxPool2D((2,2),strides=(2,2))
        self.pooling_layer2 = tf.keras.layers.MaxPool2D((2,2),strides=(2,2))
        self.pooling_layer3 = tf.keras.layers.MaxPool2D((2,2),strides=(2,2))
...
##        #image.save("image.png")
    ...
    #print(image2.shape)
##    with open("newfile.png", "w") as file:
    
        
    #file.write(image2)
    #image2.save("try.png")
##    print("file saved")
##    print("after"+str(type(image))+str(image.shape))
    image = image / 255
    return image
...
def getHuman():
    ...
    print(IMITATE)
    if IMITATE:
        with open("newfile.csv", 'r') as csv_file:
    ...
    ##            print(row_count)
            for row in csv_reader:
                ...
                for val in row:
                    ...
                    if ind1 == 0:
                        val = row['1']
                        self.mem_i.append(int(val))
                        #print(mem_i[ind2])
                        ind2+=1
                        print(val)
                    if ind1 == 1:
                        val = row['0.0']
                        self.mem_r.append(float(val))
                        #print(mem_r[ind3])
                        ind3+=1
                        #print(val)
                    if ind1 == 2:
                        val = row['0']
                        self.mem_a.append(int(val))
                        #print(mem_a[ind4])
                        ind4+=1
                        #print(val)
                        ind=0
                    ind1+=1
                    return mem_i, mem_r, mem_a
      
def get_frame(h_index):
    im = Image.open("human.gif")
    ...
    screen_data = np.empty((210, 160, 3), dtype=np.uint8)
    assert screen_data.strides == (480, 3, 1)
    #screen_data = np.ctypeslib.as_ctypes(screen_data)
    #screen_data = np.as_ctypes_type(np.dtype(screen_data))
    #screen_data = np.ctypeslib.as_ctypes(screen_data[:])
    #assert screen_data.strides == (480, 3, 1)
    ...
    #state = tf.keras.preprocessing.image.array_to_img(state)
    ...
    #state = np.reshape(state, (480, 320, 1))
    #np.squeeze(state)
##    state = Image.fromarray(state)
##    state = np.asarray(state)
    #print(state.shape)
##    next_state = im.seek(h_index)
##    next_state = np.array(next_state)
    #print(state)
    h_index+=1
    return state, h_index
#print(type(env.reset()))
c_index=0
mem_i, mem_r, mem_a = getHuman()
for i in range(num_episodes):
    c_index+=1
    if IMITATE and h_index >= 1:
        print(h_index)
        state = env.reset()
        state, h_index = get_frame(h_index)
        #print("state"+str(state.shape))
        if(h_index > row_count):
            IMITATE=False
            print("Done replaying human file, switching to autolearning")
    else:
        state = env.reset()
        #print("state"+str(state.shape))

    
    #print("heyo "+str(type(state)))
    ...
    avg_loss = 0
    tot_reward = 0
    if i % GIF_RECORDING_FREQ == 0 and IMITATE != True:
        frame_list = []
    while True:
        if render:
            env.render()
        ...
        #print(h_index)
        if IMITATE and h_index >= 1:
            next_state, h_index = get_frame(h_index)
            b_index+=1
            #print(next_state)
            #print(self.mem_a)
            action = mem_a[h_index]
            fake1, fake2, done, info = env.step(action)
            reward = mem_r[h_index]
            #print("recorded action")
            if b_index != cnt:
                print("Warning low "+str(h_index)+" "+str(b_index)+" "+str(old_h)+" "+str(cnt))
            if b_index != (h_index -old_h):
                print("Warning low2 "+str(h_index)+" "+str(b_index)+" "+str(old_h)+" "+str(cnt))
        else:
        ...
        if steps > DELAY_TRAINING or steps >=row_count:
