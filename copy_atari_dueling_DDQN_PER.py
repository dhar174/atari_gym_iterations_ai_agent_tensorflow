...
##THIS SCRIPT IS FOR EXPERIMENTING WITH RELOADING HUMAN EXPERIENCE FOR REPLAY
...
if(NEW):
    ...
    primary_network = tf.keras.models.load_model(STORE_PATH + "/checkpoints2/human")
    target_network = tf.keras.models.load_model(STORE_PATH + "/checkpoints2/human")
...
def replayHumanAction(ii):
    ...
    with open("saved_memory.csv") as file:
        ...
        header = next(reader)
        rows = [[row[0], float(row[1]),str(row[2])] for row in reader if row]
        #print(rows)
        rows[ii][2] = str(rows[ii][2])[1:-1]
        a_replay=int(rows[ii][2])
        print(a_replay)
##        for row in rows:
        #print(row[2])
##            new_str = re.sub('[^a-zA-Z0-9\n\.]', " ", str(row))
##            new_str = str(row).replace('\[', '')
##            new_str = str(row).replace(']', '')
##            new_str = str(row).replace('\'', '')
##            new_str = str(row).replace('[', '')
##            val = new_str
...
def replayHumanReward():
    ...
    with open("saved_memory.csv") as file:
        ...
        header = next(reader)
        rows = [[row[0], float(row[1]),str(row[2])] for row in reader if row]
        #rows[ii][1] = str(rows[ii][1])[1:-1]
        print(rows[ii][1])
        r_replay=float(rows[ii][1])
##        for row in rows:
##            #print(row[1])
##            print(row[1])
##            r_replay = row[1]
...
def replayHumanState():
    ...
    with open("saved_memory.csv") as file:
        ...
        #header = next(reader)
        rows = [[row[0], float(row[1]),str(row[2])] for row in reader if row]
        rows[ii][0] = str(rows[ii][0])[-1:]
        #print(rows[ii])
        #print(rows[ii][0])
        s_replay=int(rows[ii][0])
##        for row in rows:
##            print(row[0])
##            s_replay = row[0]
    #ii+=1
...
def choose_action(state, primary_network, eps, step, has_human_file = False):
...
##        frame_list = []
    while True:
        if render:
            env.render()
        action = choose_action(state_stack, primary_network, eps, steps, has_human_file)
        if has_human_file:
            next_state, reward, done, info = env.step(action)
            reward= replayHumanReward()
            ...
            try:
                ...
                try:
                    ...
                    try:
                        print("error")
                        next_state, reward, done = state, replayHumanReward(), True
                    except:
                        pass
        ii+=1
        #print(ii)
        if(ii != replayHumanState()-1):
...
##        if steps > DELAY_TRAINING:
        ...
        if has_human_file:
            if ii < row_count:
                memory.append((next_state, action, replayHumanReward(), done), error[0])
        else:
