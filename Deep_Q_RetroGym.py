...
env = retro.make(game='FamilyDog-Snes', record='.')
...
class DQModel(keras.Model):
    def __init__(self, hidden_size: int, num_actions: int, dueling: bool):
        ...
        self.conv1 = keras.layers.Conv2D(16, (8, 8), (4, 4), activation='relu',use_bias=True)
        self.conv2 = keras.layers.Conv2D(32, (4, 4), (2, 2), activation='relu',use_bias=True)
        ...
        self.adv_dense1 = keras.layers.Dense(hidden_size, activation='relu',use_bias=True,
                                         kernel_initializer=keras.initializers.he_normal())
        self.noise_layer = keras.layers.GaussianNoise(0.1)
        self.adv_dense2 = keras.layers.Dense(hidden_size, activation='softmax',use_bias=True,
                                         kernel_initializer=keras.initializers.he_normal())
        self.adv_out = keras.layers.Dense(num_actions,
                                          kernel_initializer=keras.initializers.he_normal())
        if dueling:
            self.v_dense = keras.layers.Dense(hidden_size, activation='relu',use_bias=True,
                                         kernel_initializer=keras.initializers.he_normal())
...
primary_network = DQModel(512, num_actions, True)
target_network = DQModel(512, num_actions, True)
...
def record_gif(frame_list, episode, fps=50):
    imageio.mimsave(STORE_PATH + f"/FAM_DOG_EPISODE-{episode}.gif", frame_list, fps=fps) #duration=duration_per_frame)
...
for i in range(num_episodes):
    ...
    while True:
        ...
        if done:
            ...
            if i % CHK_FREQ == 0:
                primary_network.save_weights(STORE_PATH+f"/famdog-{i}/", overwrite=True)
            if i % GIF_RECORDING_FREQ == 0:
                record_gif(frame_list, i)
            break

        cnt += 1
primary_network.save(STORE_PATH+f"/famdog-finished/") 
env.close()
