...
class DQModel(keras.Model):
    def __init__(self, hidden_size: int, num_actions: int, dueling: bool):
        ...
        self.conv1 = tf.keras.layers.Conv2D(16, (8, 8), (4, 4), padding="same",activation='relu',use_bias=False)
        self.conv2 = tf.keras.layers.Conv2D(32, (4, 4), (2, 2), padding="same",activation='relu',use_bias=False)
        self.conv3 = tf.keras.layers.Conv2D(64, (4, 4), (2, 2), padding="same",activation='relu',use_bias=False)
        self.conv4 = tf.keras.layers.Conv2D(32, (4, 4), (2, 2), padding="same",activation='relu',use_bias=False)
...
#primary_network.load_weights(tf.train.latest_checkpoint(STORE_PATH + "/checkpoints/"))
#target_network.load_weights(tf.train.latest_checkpoint(STORE_PATH + "/checkpoints/"))
...
def record_gif(frame_list, episode, fps=50):
    imageio.mimsave(STORE_PATH + f"/KUNGFU_EPISODE-{episode}.gif", frame_list, fps=fps) #duration=duration_per_frame)ation_per_frame)
...
for i in range(num_episodes):
    ...
    print(state.shape)
    state_stack = tf.Variable(np.repeat(state, NUM_FRAMES))
    cnt = 1
    avg_loss = 0
    tot_reward = 0
    ...
    while True:
        ...
        if steps > DELAY_TRAINING:
            ...
            _, error = get_per_error(old_state_stack,
                                     np.array([action]), np.array([reward]), 
                                     state_stack, np.array([done]),primary_network, target_network)
            # store in memory
    ...
    if i % MODEL_SAVE_FREQ == 0: # and i != 0:
        primary_network.save_weights(STORE_PATH + "/checkpoints/cp_primary_network_episode_{}.ckpt".format(i), overwrite=True)
        target_network.save_weights(STORE_PATH + "/checkpoints/cp_target_network_episode_{}.ckpt".format(i), overwrite=True)
